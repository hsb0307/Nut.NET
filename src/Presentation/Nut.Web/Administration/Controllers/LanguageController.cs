    ...
    public class LanguageController : BaseAdminController {
        ...
        protected virtual void PrepareFlagsModel(LanguageModel model) {
            ...
            model.FlagFileNames = Directory
                .EnumerateFiles(_webHelper.MapPath("~/Administration/Content/Images/flags/"), "*.png", SearchOption.TopDirectoryOnly)
        ...
        public ActionResult Delete(int id) {
            ...
            var allLanguages = _languageService.GetAllLanguages();
            if (allLanguages.Count == 1 && allLanguages[0].Id == language.Id) {
                return Json(new { success = false, message = _localizationService.GetResource("At least one published language is required."), });
            }

            //delete
            _languageService.DeleteLanguage(language);

            return Json(new { success = true, message = _localizationService.GetResource("Admin.Configuration.Languages.Deleted"), });
